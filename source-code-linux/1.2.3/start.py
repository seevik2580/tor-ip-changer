...
class IpChanger(Tk):
    ...
    def __init__(self):  
        Tk.__init__(self)
        #kontrola souboru icony a tail pro zobrazeni logu live, pokud neexistuje tak stahni
        ...
        if args.help is True:
            print("""usage: ipchanger [-h] [-a AUTO] [-d] [-m 1-100] [-p] [-c COUNTRY] [-g]
    ...
    #funkce na spusteni tail a vypis logu live        
    def showlog(self):
    ...
    #funkce na tail
    def tail(self):
        os.system("tail -q -f %s %s" % (logchanger, logtor))
    ...
    def API(self):
        ...
        try:
            ...
            self.write('API server binded to %s:%s (%s) ERROR:%s\n' % (self.appHOST,self.appPORT,self.bindtype,msg), "red", 1)
            return
    ...
    def prestart(self):
        #self.startcheck() 
        #self.starttor()
        _thread.start_new_thread(self.starttor, ())
