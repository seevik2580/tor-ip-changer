...
class IpChanger(Tk):
    ...
    def tailchanger(self):
        try:
            if not os.path.exists(self.tailexe):
                self.download("https://github.com/seevik2580/tor-ip-changer/raw/master/tor/%s" % (self.tailexe), 1)
    ...
    def multiTor(self, identify=None):
        ...
        if eval("self.lang_%s.get()" % str(identify)) == "{random}":
            ...
            options = "--StrictNodes 1 --ExitNodes %s" % language
        subprocess.Popen(r'Tor/tor.exe %s --RunAsDaemon 1 --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s' % (self.bridge, self.get_controlPasswordHash(identify),self.control,self.proxy,self.data,self.tordebuglevel,options), startupinfo=info, stdout = open(files, 'w'))                
    ...
    def tor(self):
        ...
        if args.multi is not None:
            ...
            if self.lang_0.get() == "{random}":
                ...
                options = "--StrictNodes 1 --ExitNodes %s" % self.lang_0.get()
            subprocess.Popen(r'Tor/tor.exe %s --RunAsDaemon 1 --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --DNSport 53 --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s' % (self.bridge, self.get_controlPasswordHash('0'), self.control,self.proxy,self.data,self.tordebuglevel,options), startupinfo=info, stdout = open(file, 'w'))
    ...
    def stop(self): 
        ...
        self.write('Resolving IP stopping. Wait..\n', "error", 1)
        self.ident = random.random() 
        self.meni=0
        self.IPchanged = 0
        self.IPfetched = 0
    ...
    def ip(self):
        self.write("Resolving IP started.\n","green", 1)
        key = self.ident
        while key == self.ident:
            if key != self.ident:
                break
        ...
        self.write("Resolving IP stopped.\n","error", 1) 
        self.meni=0
        self.IPchanged = 0
        self.IPfetched = 0
        self.failed=0
