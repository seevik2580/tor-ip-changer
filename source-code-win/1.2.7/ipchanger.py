...
import tkinter.ttk, random, _thread, time, tkinter.scrolledtext, socket, re, logging, os, sys, multiprocessing, \
    subprocess, pycurl, io, argparse, urllib3, string, json, urllib.request, configparser
from tkinter import *
...
parser.add_argument("-a", "--auto", type=int, required=False,
                    help="-a 10   change ip every 10 seconds after program start")
parser.add_argument("-d", "--debug", help="start debug logging", action='store_true')
parser.add_argument("-m", "--multi", type=int, required=False, help="-m 5   run 5 different TOR instances",
                    choices=range(1, 101))
parser.add_argument("-p", "--publicAPI", action='store_true', required=False,
                    help="bind API to 0.0.0.0 instead of 127.0.0.1")
parser.add_argument("-c", "--country", type=str, required=False,
                    help="-c cz,sk,us   select czech republic, slovakia and united states country")
    ...
    def configwindow(self):
        ...
        if args.multi is not None:
            if self.forall == 1:
                ...
                self.checkforall = Checkbutton(self.newWindow, text="for all?", variable=self.checkvar, onvalue=1,
        ...
        if args.multi is not None:
            ...
            for i in range(args.multi):
                ...
                Label(self.newWindow, text='%s:%s' % (self.host.get(), proxy + i)).grid(row=r, column=cl - 4,
                                                                                        padx=(10, 10))
                ...
                exec('self.tor_bridge_yes_' + str(
                    i) + ' = Radiobutton(self.newWindow, text = "Yes", variable = self.useBridges_' + str(
                    i) + ', value = True, width = 3)')
                exec('self.tor_bridge_yes_' + str(i) + '.grid(row=r,column=cc+1-4)')

                exec('self.tor_bridge_no_' + str(
                    i) + ' = Radiobutton(self.newWindow, text = "No", variable = self.useBridges_' + str(
                    i) + ', value = False, width = 3)')
            ...
            self.tor_bridge_yes_0 = Radiobutton(self.newWindow, text="Yes", variable=self.useBridges_0, value=True,
                                                width=3)
            self.tor_bridge_yes_0.grid(row=r, column=cc + 1 - 4)
            self.tor_bridge_no_0 = Radiobutton(self.newWindow, text="No", variable=self.useBridges_0, value=False,
                                               width=3)
...
usage: ipchanger.exe [-a AUTO] [-d] [-m 1-100] [-p] [-c COUNTRY] [-b] [-n]
    ...
    def tailchanger(self):
        try:
            if not os.path.exists(self.tailexe):
                self.download(
                    "https://raw.githubusercontent.com/seevik2580/tor-ip-changer/master/tor/%s" % (self.tailexe), 1)
    ...
    def multiTor(self, identify=None):
        ...
        subprocess.Popen(
            r'Tor/tor.exe %s --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s' % (
            self.bridge, self.get_controlPasswordHash(identify), self.control, self.proxy, self.data,
            self.tordebuglevel, options), startupinfo=info, stdout=open(files, 'a'))
    ...
    def tor(self):
        ...
        if args.multi is not None:
            ...
            if self.lang_0.get() == "random":
                ...
                self.debug(options)
            subprocess.Popen(
                r'Tor/tor.exe %s --CookieAuthentication 0 --SocksPolicy "accept *" --HashedControlPassword "%s" --ControlPort %s --SocksPort 0.0.0.0:%s --DataDirectory %s --log %s --AvoidDiskWrites 1 --SafeLogging 0 --GeoIPExcludeUnknown 1 --GeoIPFile Tor/geoip --GeoIPv6File Tor/geoip6 --DNSport 53 --AutomapHostsSuffixes .onion --AutomapHostsOnResolve 1 %s' % (
                self.bridge, self.get_controlPasswordHash('0'), self.control, self.proxy, self.data, self.tordebuglevel,
                options), startupinfo=info, stdout=open(file, 'a'))
        ...
        while not timeout == tout and key == self.ident:
            ...
            try:
                ...
                circuitstatus = self.torGetInfo(15000, self.get_controlPassword(0),
    ...
    def update(self):
        try:
            if self.noUpdate is False:
                ...
                http = urllib3.PoolManager()
                f = http.request('GET',
                                 'https://raw.githubusercontent.com/seevik2580/tor-ip-changer/master/version.txt')
                ...
                f = http.request('GET',
                                 "https://raw.githubusercontent.com/seevik2580/tor-ip-changer/master/dist/%s/build.txt" % self.lastver)
    ...
    def IPandlatency(self, port=None):
        ...
        def fetching(proxy):
            ...
            try:
                ...
                cport = 5950 + proxy
                iptocountry = self.torGetInfo(cport, self.get_controlPassword(languagenumber),
                                              "GETINFO ip-to-country/%s" % ip)
                ...
                self.write(
                    "|" + host.ljust(9) + ":" + str(proxy).ljust(5) + "|" + str(language).ljust(12) + "-> " + str(
                        ip).rjust(16) + " |" + str(country[1]) + "|" + str(latency).rjust(6) + "|\n", color, 0)
                ...
                if len(language) > 6:
                    language = language.replace(language[6:], "...")
                self.write("|" + host.ljust(9) + ":" + str(proxy).ljust(5) + "|" + str(language).ljust(10) + " " + str(
                    status).rjust(1) + "|\n", "error", 0)
